...
更新日期：2026-09-20。当前有效状态以本节、`development-order.json` 与 `modules.json` 为准；下方按日期保留的日志记录当时状态，不能据此覆盖后续发行或用户决定。

## 2026-09-20：传神可控推演写作插件候选

传神插件已从 28 个工具扩展为 46 个工具，增加真实语料包、借形不借值 Skeleton、继承 Diff、项目/事实治理、Chunk 依据、编辑语义 Diff、事实变更回滚、版本比较和导出状态。插件仍只通过传神 FastAPI 工作，不连接 PostgreSQL、MinIO、OpenSearch、Qdrant、FalkorDB、Redis 或 RabbitMQ。受控值修改必须先预览再选择应用；未绑定精确数字阻断，未绑定语义相似文本只作为疑似影响。

插件 build、typecheck 和 8 项专项测试通过。服务器安装、真实 `deepseek-v4-flash-0731` 多步骤任务和浏览器闭环尚未在本状态记录时完成，不能把源码候选写成生产验收通过。
...
## 2026-09-18：传神智库解析、推演、测算与写作插件（进行中）

按用户明确授权新增 `workdsh-plugin-chuanshen`，通过官方 `defineTool()`、Cordis 生命周期和 System Prompt 扩展接入传神智库，不新增第二套 Agent Loop、模型路由或业务数据库。当前实现 20 项工具，覆盖空间/文档、真实文件上传、后台任务跟踪、全文/向量/知识图谱/写作图谱加工、检索、写作知识治理读取、Semantica 规则推演、确定性测算、妙笔项目上下文、分章节生成、事实变更影响预览/选择应用、文稿检查及导出任务。解析、索引、规则、公式、文稿版本与导出仍以传神智库为权威服务。

平台账号密码存放在仓库外 `0600` 文件，Host 登录后只在内存持有短期 Bearer；上传工具只能读取显式允许目录。发布推演、应用事实影响和创建导出要求显式确认。客户端单测 3/3 通过，覆盖 401 刷新且不向业务请求泄露密码、真实文件上传/目录越界拒绝、凭据权限过宽拒绝；包类型检查、构建和打包通过。`check:plan` 当前仍被本分支基线已有的专家资源和历史文档缺失阻断，与本插件新增文档无关，尚未据此标记整体规划通过。

尚待：服务器安装、模型端点校准、真实平台工具调用、真实模型 Agent 回合、浏览器验证、凭据轮换/专用最小权限账号以及 GitHub 推送。未执行项不写成已通过。

## 2026-09-20：修复 DSH Web 全文件上传失败

真实浏览器使用 114B TXT 稳定复现“上传失败”。首层根因是 DSH 0.1.6-alpha.1 的默认 Blob Worker 传输在创建后立即撤销对象 URL，当前 Chromium 只请求 `blob:` 地址而未执行 Worker，官方 `/api/session/uploadFileBinary` 从未收到请求。`workdsh-bundle@0.1.0-alpha.46` 通过官方 `__DSH_FILE_UPLOAD__` 页面扩展点在 Cordis 启动前安装同源 Fetch 载体，不修改上游包或 `node_modules`。针对注入顺序、单次注入、请求透传、浏览器凭据和已有宿主通道保护的 3 项测试，加组合包生命周期 1 项测试，共 4/4 通过。

部署后继续抓取真实响应，发现第二层根因：18990 返回 HTTP 200，但业务体为 `gateway/internal`，底层错误是运行用户 `workdsh` 无权读取 `/home/tianqi`。官方附件存储为保证不可变对象持久化会打开并同步 `DSH_HOME` 的父目录，原目录只有穿越权限不足。服务器已备份原 ACL 到 `/home/tianqi/backups/dsh/home-tianqi-before-upload-fix-20260920.acl`，并只为 `workdsh` 增加 `r-x` ACL；没有向其他用户开放目录。候选包经官方 Profile 安装到 10.5.113.232，服务重启后仍为 `0.1.0-alpha.46`。

真实浏览器回归：114B TXT、976B PDF、126757B 中文文件名 PDF 均显示“待发送文件”，官方附件目录生成对应内容寻址对象和显示名别名，发送按钮恢复可用；测试附件随后从当前会话草稿移除，未发送消息。正常重启 DSH 后再次上传 114B TXT 仍成功。当前结论只证明 Web 附件暂存链路；把附件交给模型后的解析质量和不同超大文件限制不在本轮验证范围。
...
## 2026-09-20：同步 dshwork 上游 0.1.0-alpha.6
