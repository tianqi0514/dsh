...
按用户明确授权新增 `workdsh-plugin-chuanshen`，通过官方 `defineTool()`、Cordis 生命周期和 System Prompt 扩展接入传神智库，不新增第二套 Agent Loop、模型路由或业务数据库。当前实现 18 项工具，覆盖空间/文档、真实文件上传、全文/向量/知识图谱/写作图谱加工、检索、写作知识治理读取、Semantica 规则推演、确定性测算、妙笔项目上下文、分章节生成、事实变更影响预览/选择应用、文稿检查及导出任务。解析、索引、规则、公式、文稿版本与导出仍以传神智库为权威服务。
