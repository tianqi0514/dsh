...
| 官方能力 | `docs/deepseek-harness-docs/cordis-tutorial/07-into-the-harness.md`、`cookbook/adding-a-tool.md`、`subsystems/tools.md`；锁定 `@deepseek-ai/dsh-tools@0.1.6-alpha.1`、`@deepseek-ai/dsh-system-prompt@0.1.6-alpha.1`、Cordis `4.0.2`。 |
| 复用选择 | 使用 `ctx.tools.register(defineTool(...))` 注册工具，使用 `ctx.systemPrompt.section()` 追加工作流约束，继续由官方 Agent Loop、模型路由、Session 日志、Conversation renderer 和 Profile Loader 运行。 |
...
- 版本：`0.1.0-alpha.1`
- 工具范围：材料接入与四路加工、知识检索、写作图谱治理、规则推演、确定性测算、写作任务、事实变更和导出。
- 本地类型检查、构建和打包通过；客户端单测 3/3 通过。
