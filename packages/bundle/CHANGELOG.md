## 0.1.0-alpha.53 — 2026-09-20

- 在冷启动的 15 秒稳定窗口内持续保持入口指定的业务面板，兼容 DSH 首次运行声明较晚重置活动面板的情况；窗口结束后仍恢复正常用户导航。

## 0.1.0-alpha.52 — 2026-09-20

- 在 DSH 首次运行声明改写地址前捕获产品视图，声明显示完毕后继续恢复传神智库工作台，而不是丢失用户打开的入口。

## 0.1.0-alpha.51 — 2026-09-20

- 冷启动时将已知业务面板的 URL 恢复等待窗口延长至 15 秒，避免 DSH 内测声明与插件分阶段加载期间把 `workdsh-view=chuanshen` 过早重置为会话页。

## 0.1.0-alpha.50 — 2026-09-20

- 已知业务面板在 Client 分阶段启动时最多等待两秒完成注册，再恢复 URL；修复直接刷新 `workdsh-view=chuanshen` 偶发回到对话页的问题。
- 未安装或未注册的页面仍按原契约安全回退到原生对话，不产生空白主区域。

## 0.1.0-alpha.49 — 2026-09-20

- 将 `chuanshen` 纳入产品 URL 到原生面板的正式映射；进入传神智库后 URL 可恢复，刷新和从已有会话切换不会退回对话页。
- 配套 `workdsh-plugin-chuanshen@0.1.0-alpha.3`，修正只返回分状态数量时写作图谱总数显示为零的问题。
